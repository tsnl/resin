...
fn instances_are_memoized_and_the_exact_allowance_is_admitted() {
    ...
    let lir = support::build_lir(&hir, &[], &options(2)).unwrap();
    ...
    let error = support::build_lir(&hir, &[], &options(1))
        .unwrap_err()
        .remove(0);
    assert_eq!(
        error.kind,
...
fn substitution_normalizes_unions_before_memoization() {
    ...
    let lir = support::build_lir(&hir, &[], &options(1)).unwrap();
...
fn growing_recursive_requests_fail_with_a_bounded_application_trace() {
    ...
    let errors = support::build_lir(&hir, &[], &options(40)).unwrap_err();
...
fn identity_signatures_and_bodies_are_concrete_without_changing_hir() {
    ...
    let lir = support::build_lir(&hir, &[], &resin_lir::LoweringOptions::default()).unwrap();
    resin_lir::verify(&lir).unwrap();
...
fn unused_families_do_not_constrain_supported_concrete_operations() {
    ...
    assert_eq!(
        support::build_lir(&hir, &[], &resin_lir::LoweringOptions::default())
            .unwrap()
            .functions
            .len(),
        1
    ...
    let error = support::build_lir(&hir, &[], &resin_lir::LoweringOptions::default())
        .unwrap_err()
        .remove(0);
...
fn failed_requests_are_memoized_without_publishing_an_incomplete_module() {
    ...
    let errors = support::build_lir(&hir, &[], &options(1)).unwrap_err();
    assert_eq!(errors.len(), 1);
...
fn requested_roots_exclude_unused_functions_types_and_drop_hooks() {
    ...
    assert!(
        support::build_lir(&hir, &[], &resin_lir::LoweringOptions::default()).is_err(),
...
fn explicit_root_arguments_normalize_and_preserve_recursive_nominal_identity() {
    ...
    let lir = support::build_lir(&hir, &[root, duplicate], &options(1)).unwrap();
    resin_lir::verify(&lir).unwrap();
...
fn host_and_shader_requests_have_separate_profiles_and_both_count_toward_the_limit() {
    ...
    let lir = support::build_lir(&hir, &roots, &options(2)).unwrap();
    resin_lir::verify(&lir).unwrap();
    ...
    let error = support::build_lir(&hir, &roots, &options(1))
        .unwrap_err()
        .remove(0);
    assert!(matches!(
        error.kind,
...
fn nominal_expansion_is_bounded_across_declaration_boundaries() {
    ...
    let error = support::build_lir(&hir, &[root], &options(1))
        .unwrap_err()
        .remove(0);
    assert!(matches!(error.kind, ErrorKind::TypeExpansionLimit { .. }));
}
...
fn nominal_arguments_have_identity_without_demanding_their_layout() {
    ...
    let lir = support::build_lir(&hir, &requests, &options(2)).unwrap();
    resin_lir::verify(&lir).unwrap();
    assert_eq!(lir.functions.len(), 2);
    assert!(lir.types.is_empty());
    let error = support::build_lir(&hir, &requests, &options(1))
        .unwrap_err()
        .remove(0);
    assert!(
...
fn nominal_instances_substitute_fields_and_close_recursive_edges() {
    ...
    measure_parameter(&mut hir);
    let lir = support::build_lir(&hir, &nominal_roots(), &options(2)).unwrap();
    resin_lir::verify(&lir).unwrap();
...
fn nominal_recursion_with_growing_arguments_reports_a_type_limit() {
    ...
    measure_parameter(&mut hir);
    let error = support::build_lir(&hir, &nominal_roots()[..1], &options(1))
        .unwrap_err()
        .remove(0);
    assert!(
...
fn shader_dependency_order_is_iterative_and_rejects_cycles_with_bounded_notes() {
    ...
    let lir = support::build_lir(&hir, &roots, &options(1)).unwrap();
...
fn generic_conversions_cannot_bypass_custom_destruction() {
    ...
    let error = support::build_lir(&hir, &[request], &LoweringOptions::default())
        .unwrap_err()
        .remove(0);
    assert!(matches!(
        error.kind,
...
fn build_lir_method(hir: &Module) -> Result<resin_lir::Module, Vec<resin_lir::Error>> {
    ...
    support::build_lir(hir, &[request], &options(1))
}

...
pub(super) fn check_definitions(table: &TypeTable) -> Result<(), VerifyError> {
    for index in 0..table.len() {
        ...
        if let TypeDef::Structural(ty) = &table[index] {
            ...
            resin_types::check_layout(table, id, body)
...
pub(super) fn check_type(
    ...
    resin_types::check_references(table, ty).map_err(|error| location.error(error.into()))
}
...
fn definition_body(
    ...
    resin_types::definition_body(table, definition).map_err(|error| location.error(error.into()))
}
...
pub(super) fn ascribe(
    ...
    if resin_types::ascription(table, &found, expected)
        .map_err(|error| location.error(error.into()))?
        .is_some()
    {
        return Ok(());

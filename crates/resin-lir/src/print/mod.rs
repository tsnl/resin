...
fn sexp_instr(names: &Names, fn_names: &FunctionNames, instr: &Instr) -> SExp {
    match instr {
        ...
        Instr::GpuBufferLoad { element } => list("gpu-buffer-load", vec![sexp_ty(names, element)]),
        Instr::GpuBufferStore => symbol("gpu-buffer-store"),
...
fn sexp_value(names: &Names, value: &Value) -> SExp {
    match value {
        ...
        Value::Int8 { value } => list("i8", vec![symbol(value.to_string())]),
        Value::Int16 { value } => list("i16", vec![symbol(value.to_string())]),
        Value::Int32 { value } => list("i32", vec![symbol(value.to_string())]),
        Value::Int64 { value } => list("i64", vec![symbol(value.to_string())]),
        Value::UInt8 { value } => list("u8", vec![symbol(value.to_string())]),
        Value::UInt16 { value } => list("u16", vec![symbol(value.to_string())]),
        Value::UInt32 { value } => list("u32", vec![symbol(value.to_string())]),
        Value::UInt64 { value } => list("u64", vec![symbol(value.to_string())]),
        Value::Float32 { value } => list("f32", vec![symbol(value.to_string())]),
        Value::Float64 { value } => list("f64", vec![symbol(value.to_string())]),
...
fn sexp_ty(names: &Names, ty: &Ty) -> SExp {
    match ty {
        ...
        Ty::Int64 => symbol("i64"),
        Ty::UInt8 => symbol("u8"),
        Ty::UInt16 => symbol("u16"),
        Ty::UInt32 => symbol("u32"),
        Ty::UInt64 => symbol("u64"),
        Ty::Float32 => symbol("f32"),
        Ty::Float64 => symbol("f64"),
        Ty::Str => symbol("str"),

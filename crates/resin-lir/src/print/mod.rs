...
fn sexp_instr(names: &Names, fn_names: &FunctionNames, instr: &Instr) -> SExp {
    match instr {
        ...
        Instr::ArcSpanData => symbol("arc-span-data"),
        Instr::ArcSpanTryNew { element } => list("arc-span-try-new", vec![sexp_ty(names, element)]),
        Instr::HostAllocate { error, element } => list(
            "host-allocate",
            vec![
                symbol(names.functions[error.index()].as_ref()),
                sexp_ty(names, element),
            ],
        ),
        Instr::SpanBytes => symbol("span-bytes"),
        Instr::SpanSlice => symbol("span-slice"),
        Instr::Downgrade => symbol("downgrade"),
        Instr::Upgrade => symbol("upgrade"),
        Instr::WeakEmpty { ty } => list("weak-empty", vec![sexp_ty(names, ty)]),
...
fn sexp_ty(names: &Names, ty: &Ty) -> SExp {
    match ty {
        ...
        Ty::ArcPtr { pointee } => list("arc-ptr", vec![sexp_ty(names, pointee)]),
        Ty::WeakPtr { pointee } => list("weak-ptr", vec![sexp_ty(names, pointee)]),
        Ty::ArcSpan { element } => list("arc-span", vec![sexp_ty(names, element)]),
        Ty::WeakSpan { element } => list("weak-span", vec![sexp_ty(names, element)]),

...
fn recovering_valid_syntax_matches_strict_generation() {
    ...
    let document = Document::build(source.into(), None);
    let recovered = resin_ast::recover(&document);
    let strict = resin_ast::build_ast(&document).unwrap();
    assert!(recovered.errors.is_empty());
    assert_eq!(
...
fn source_locations_tolerate_editor_offsets_inside_utf8() {
    ...
    let module = SourceModule {
        ...
        file: resin_ast::build_ast(&Document::build(source.into(), None)).unwrap(),
        imports: vec![],
    };
    assert_eq!(
...
fn generic_wrapper_types_lower_with_their_element_annotations() {
    ...
    let file = resin_ast::build_ast(&Document::build(source.into(), None)).unwrap();

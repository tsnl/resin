...
fn options(environment: &Environment, profile: BuildProfile) -> Options {
    Options {
...
fn request(
    ...
    Request::new(input, destination, options)
}
...
fn requests_reject_source_overwrites_before_compilation() {
    ...
    for destination in [&source, &alias] {
        let result = request(
            ...
            options(&environment, BuildProfile::Debug),
        );
        let error = result
            .err()
...
fn requests_resolve_executable_directories_and_preserve_file_destinations() {
    ...
    for directory in [temp.path().to_path_buf(), temp.path().join("new/")] {
        let request = request(
            ...
            options(&environment, BuildProfile::Release),
        )
        .unwrap();
        assert_eq!(

...
use crate::tensor::{IndexKeyElement, Tensor, TensorKind};
...
pub fn shift_axis(x: &Tensor, axis: usize, offset: usize, fill: &Tensor) -> Tensor {
    ...
    if offset >= n {
        ...
        let axes: Vec<usize> = Vec::new();
        return fill.broadcast_to(shape, &axes);
    }
    ...
    let axes: Vec<usize> = Vec::new();
    let front_region = fill.broadcast_to(&front_shape, &axes);
...
fn zero_scalar(like: &Tensor) -> Tensor {
    match like.element_type() {
        crate::tensor::ElementType::U32 => Tensor::full_u32(&[], 0),
        _ => Tensor::full(&[], 0.0, like.element_type()),
    }
}

fn one_scalar(like: &Tensor) -> Tensor {
    match like.element_type() {
        crate::tensor::ElementType::U32 => Tensor::full_u32(&[], 1),
        _ => Tensor::full(&[], 1.0, like.element_type()),
    }
}

fn is_zero_constant(t: &Tensor) -> bool {
    matches!(t.kind(), TensorKind::Constant { bytes } if bytes.iter().all(|&b| b == 0))
}
...
mod tests {
    ...
    fn scan_preserves_shape_and_type() {
        let x = Tensor::parameter(&[7], ElementType::F32);
        let out = cumsum(&x, 0);
    ...
    #[test]
    fn scan_on_u32() {
        let x = Tensor::parameter(&[8], ElementType::U32);
        let out = cumsum(&x, 0);
    ...
    fn scan_along_leading_axis_of_matrix() {
        let x = Tensor::parameter(&[5, 3], ElementType::F32);
        let out = cumprod(&x, 0);
    ...
    fn shift_by_zero_is_identity() {
        ...
        let fill = Tensor::full(&[], 0.0, ElementType::F32);
    ...
    fn shift_past_length_is_fill() {
        let x = Tensor::parameter(&[4], ElementType::F32);
        let fill = Tensor::full(&[], 1.0, ElementType::F32);
    ...
    fn shift_rejects_non_scalar_fill() {
        let x = Tensor::parameter(&[4], ElementType::F32);
        let fill = Tensor::full(&[4], 0.0, ElementType::F32);
        shift_axis(&x, 0, 1, &fill);
    }

    #[test]
    fn scan_length_one_traces_no_steps() {
        let x = Tensor::parameter(&[1], ElementType::F32);
        let zero = Tensor::full(&[], 0.0, ElementType::F32);
        let mut calls = 0;

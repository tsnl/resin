...
pub fn generate(file: &ast::SourceFile) -> Result<lir::Module, GenerateError> {
    let hir = hir::generate(file)?;
    let lir = lir::lower::generate(&hir).map_err(|error| error.error)?;
    lir_verifier::VerifiedModule::new(lir)
        .map(|v| v.into_module())
        .map_err(invalid_lir)
}

pub fn generate_program(program: &ast::Program) -> Result<lir::Module, ast::SourceError> {
    let hir = hir::generate_program(program)?;
    lower(program, &hir)
        .map(|v| v.into_module())
...
pub(super) fn lower(
    ...
    let lir = lir::lower::analyze(hir).map_err(|errors| {
        errors
            .into_iter()

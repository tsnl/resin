...
/// Concrete tensor handle for the Vulkan backend.
///
/// Host staging bytes are kept so callers use the same upload/download
/// [`ConcreteTensor`] API as the other backends; device buffers are created
/// per invoke by the runtime.
#[doc(hidden)]
...
impl ConcreteTensor for VulkanTensor {
    fn shape(&self) -> &[usize] {
        &self.shape
    }

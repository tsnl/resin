...
use crate::types::{Env, Value, TensorVal, TensorData};
use crate::vocab;

// ---------------------------------------------------------------------------
// Panic error (comptime evaluation failure → type error)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// ComptimeEnv: functions available for compile-time evaluation
// ---------------------------------------------------------------------------
    ...
    match expr {
        ...
        Expr::Apply(app) => {
            ...
            let func_name = match &app.callee {
                Expr::Name(n) => &n.name,
                _ => return Err(panic_err("higher-order calls not yet supported in comptime")),
            };
        ...
        Expr::If(if_expr) => {
            for (cond, body) in &if_expr.cond_branch_vec {
                let cond_val = interpret(cond, env, comptime)?;
                match cond_val.as_bool() {
                    Some(true) => return interpret(body, env, comptime),
                    Some(false) => continue,
        ...
        Expr::Chain(chain) => {
            ...
            for stmt in &chain.stmt_vec {
                match stmt {
                    Stmt::Let(let_stmt) => {
                        let val = interpret(&let_stmt.init, &current_env, comptime)?;
                        let name = match &let_stmt.pattern {
                            Pattern::Name(n) => n.name.clone(),
                            _ => return Err(panic_err("unsupported pattern in comptime let")),
                        };
                    ...
                    _ => return Err(panic_err("unsupported statement in comptime")),
                }
            }
        ...
        Expr::Dot(dot) => {
            let base = interpret(&dot.base, env, comptime)?;
            match base {
...
fn literal_to_value(lit: &vocab::Literal) -> Result<Value, PanicError> {
    match lit {
        ...
        vocab::Literal::String(_) => Err(panic_err("string values not supported in comptime")),
    }
}
...
pub fn builtin_comptime_env() -> ComptimeEnv {
...
mod tests {
    ...
    fn test_interpret_literal() {
        let env = Env::empty();
        let comptime = builtin_comptime_env();
        ...
        let result = interpret(&expr, &env, &comptime).unwrap();
    ...
    #[test]
    fn test_interpret_comptime_call() {
        let env = Env::empty();
        let comptime = builtin_comptime_env();
        let d = crate::Span::dummy();
    ...
    fn test_compiled_fn_call() {
        ...
        let comptime = comptime.register(
            Symbol::from("double"),
            CompiledFn {

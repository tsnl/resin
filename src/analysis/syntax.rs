            ...
            DefinitionKind::Function => node
                ...
                .or_else(|| {
                    node.children(&mut node.walk())
                        ...
                        .map(|end| {
                            format!("{} -> ()", &self.text[node.start_byte()..end.end_byte()])
                        })
                }),
    ...
    fn visit(&mut self, path: &Path, node: Node<'_>, scope: Span, top_level: bool) {
        ...
        match node.kind() {
            ...
            "function_definition" | "foreign_function" => {
                // Recovery can absorb an invalid module variable and the next
                // function into one definition, using the variable as its name.
                // Only index a name actually followed by the parameter list.
                let mut next = node
                ...
                if !next.is_some_and(|n| n.kind() == "(" && !n.is_missing()) {
            ...
            "declare" if !top_level && node.parent().is_some_and(|p| p.kind() == "statement") => {
                self.definition(
                    path,
                    node,
                    DefinitionKind::Variable,
    ...
    pub fn in_module_type_definition(&self, offset: usize) -> bool {
        ...
        loop {
            if node.kind() == "type_define"
                && node
                    .parent()
                    .filter(|n| n.kind() == "type_definition")
                    .and_then(|n| n.parent())

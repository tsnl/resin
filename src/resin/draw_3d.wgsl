...
// Wavefront path tracing POD types:
struct PodRay {
    origin_x: f32,
    origin_y: f32,
    origin_z: f32,
...
const POD_PATH_STATE_FLAG_TERMINATED: u32 = 1u;
...
// Wavefront path tracing buffers (per-ray state):
// Double-buffered for compaction - first half is buffer 0, second half is buffer 1
...
// Compaction state: [0] = active ray count, [1] = source buffer offset, [2] = dest buffer offset
@group(1) @binding(18) var<storage, read_write> wf_compact_state: array<atomic<u32>>;
...
//
// These kernels implement the wavefront path tracing algorithm.
// Each kernel processes one stage of the path tracing pipeline.
//

// Helper: Pack pixel coordinates into a u32
fn wf_pack_pixel_id(x: u32, y: u32) -> u32 {
    return x | (y << 16u);
}

// Helper: Unpack pixel coordinates from a u32
fn wf_unpack_pixel_id(pixel_id: u32) -> vec2<u32> {
    return vec2<u32>(pixel_id & 0xFFFFu, pixel_id >> 16u);
}

// Helper: Get ray index for a pixel
fn wf_ray_index(pixel_xy: vec2<u32>) -> u32 {
    return pixel_xy.y * frame_info.target_size_w_px + pixel_xy.x;
}

// Compaction state indices
const WF_COMPACT_ACTIVE_COUNT: u32 = 0u;  // Number of active rays after compaction
const WF_COMPACT_CURRENT_BUF: u32 = 1u;   // Current buffer index (0 or 1) for ping-pong
const WF_COMPACT_NEXT_COUNT: u32 = 2u;    // Atomic counter for next bounce's active rays

// Helper: Get total ray count for a frame
fn wf_total_rays() -> u32 {
    return frame_info.target_size_w_px * frame_info.target_size_h_px;
}

// Helper: Get buffer offset for the specified buffer index
fn wf_buffer_offset(buffer_idx: u32) -> u32 {
    return buffer_idx * wf_total_rays();
}

// Helper: Get the current source buffer offset
fn wf_src_offset() -> u32 {
    let current_buf = atomicLoad(&wf_compact_state[WF_COMPACT_CURRENT_BUF]);
    return wf_buffer_offset(current_buf);
}

// Helper: Get the current destination buffer offset
fn wf_dst_offset() -> u32 {
    let current_buf = atomicLoad(&wf_compact_state[WF_COMPACT_CURRENT_BUF]);
    return wf_buffer_offset(1u - current_buf);
}

// Helper: Store a Ray into a WavefrontRay at absolute index
fn wf_store_ray_at(abs_index: u32, ray: Ray, pixel_id: u32, bounce: u32) {
    wf_rays[abs_index].origin_x = ray.origin.x;
    wf_rays[abs_index].origin_y = ray.origin.y;
    wf_rays[abs_index].origin_z = ray.origin.z;
    wf_rays[abs_index].pixel_id = pixel_id;
    wf_rays[abs_index].direction_x = ray.direction.x;
    wf_rays[abs_index].direction_y = ray.direction.y;
    wf_rays[abs_index].direction_z = ray.direction.z;
    wf_rays[abs_index].bounce = bounce;
}

// Helper: Store a Ray using local index + current dest offset
fn wf_store_ray(local_index: u32, ray: Ray, pixel_id: u32, bounce: u32) {
    wf_store_ray_at(wf_dst_offset() + local_index, ray, pixel_id, bounce);
}

// Helper: Load a Ray from absolute index
fn wf_load_ray_at(abs_index: u32) -> Ray {
    return Ray(
        vec3<f32>(
            wf_rays[abs_index].origin_x,
            wf_rays[abs_index].origin_y,
            wf_rays[abs_index].origin_z,
        ),
        vec3<f32>(
            wf_rays[abs_index].direction_x,
            wf_rays[abs_index].direction_y,
            wf_rays[abs_index].direction_z,
        ),
    );
}

// Helper: Load a Ray using local index + current src offset
fn wf_load_ray(local_index: u32) -> Ray {
    return wf_load_ray_at(wf_src_offset() + local_index);
}

// Helper: Get pixel_id from ray at absolute index
fn wf_get_pixel_id_at(abs_index: u32) -> u32 {
    return wf_rays[abs_index].pixel_id;
}

// Helper: Get pixel_id from ray using local index + current src offset
fn wf_get_pixel_id(local_index: u32) -> u32 {
    return wf_get_pixel_id_at(wf_src_offset() + local_index);
}

// Helper: Get bounce count from ray at absolute index
fn wf_get_bounce_at(abs_index: u32) -> u32 {
    return wf_rays[abs_index].bounce;
}

// Helper: Store a HitRecord into a WavefrontHit
fn wf_store_hit(ray_index: u32, hit_record: HitRecord) {
    wf_hits[ray_index].barycentric_u = hit_record.barycentric_coordinates.y;
    wf_hits[ray_index].barycentric_v = hit_record.barycentric_coordinates.z;
    wf_hits[ray_index].distance = hit_record.world_hit_distance;
    wf_hits[ray_index].triangle_id = hit_record.triangle_id;
    wf_hits[ray_index].instance_id = hit_record.instance_id;
    wf_hits[ray_index].geometry_id = hit_record.geometry_id;
}

// Helper: Load a HitRecord from a WavefrontHit (needs ray for full reconstruction)
fn wf_load_hit(ray_index: u32, ray: Ray) -> HitRecord {
    let wf_hit = wf_hits[ray_index];
    let bc_u = wf_hit.barycentric_u;
    let bc_v = wf_hit.barycentric_v;
    let bc_w = 1.0 - bc_u - bc_v;

    var hit_record: HitRecord;
    hit_record.world_hit_position = ray.origin + wf_hit.distance * ray.direction;
    hit_record.world_hit_distance = wf_hit.distance;
    hit_record.barycentric_coordinates = vec3<f32>(bc_w, bc_u, bc_v);
    hit_record.ray = ray;
    hit_record.triangle_id = wf_hit.triangle_id;
    hit_record.geometry_id = wf_hit.geometry_id;
    hit_record.instance_id = wf_hit.instance_id;
    return hit_record;
}

// Kernel 1: Generate primary rays for all pixels
@compute @workgroup_size(8, 8, 1)
fn wf_gen_primary_rays(@builtin(global_invocation_id) global_id: vec3<u32>) {
    if global_id.x >= frame_info.target_size_w_px || global_id.y >= frame_info.target_size_h_px {
        return;
    }

    let pixel_xy = global_id.xy;
    let pixel_coords = vec2<i32>(pixel_xy);

    // Clear debug output textures at the start of each frame
    textureStore(frame_surface_depth_image, pixel_coords, vec4<f32>(0.0));
    textureStore(frame_surface_position_image, pixel_coords, vec4<f32>(0.0));
    textureStore(frame_surface_color_image, pixel_coords, vec4<f32>(0.0));
    textureStore(frame_surface_normal_image, pixel_coords, vec4<f32>(0.0));
    textureStore(frame_surface_orm_image, pixel_coords, vec4<f32>(0.0));

    let ray_index = wf_ray_index(pixel_xy);
    let pixel_id = wf_pack_pixel_id(pixel_xy.x, pixel_xy.y);

    // Generate primary ray with jitter for the first sample
    // For multi-sample, we generate rays for sample 0 here; subsequent samples handled in shade_and_extend
    var jitter = vec2<f32>(0.0);
    if (frame_info.debug_flags & FLAG_DISABLE_JITTER) == 0u {
        jitter = compute_primary_ray_jitter(0u);
    }
    let ray = gen_primary_ray_jittered(pixel_xy, jitter);

    // Debug hook for primary ray visualization
    debug_hook_post_primary_ray_gen(ray, vec2<i32>(pixel_xy));

    // Store ray to buffer 0 (primary rays always start in buffer 0)
    // Use wf_store_ray_at to write directly without reading compact_state
    wf_store_ray_at(ray_index, ray, pixel_id, 0u);

    // Initialize path state (indexed by pixel, not compacted)
    let pixel_index = ray_index;  // For primary rays, ray_index == pixel_index
    let seed = compute_pixel_seed(pixel_xy);
    wf_path_states[pixel_index].throughput_r = 1.0;
    wf_path_states[pixel_index].throughput_g = 1.0;
    wf_path_states[pixel_index].throughput_b = 1.0;
    wf_path_states[pixel_index].rng_seed = seed;
    wf_path_states[pixel_index].accumulated_r = 0.0;
    wf_path_states[pixel_index].accumulated_g = 0.0;
    wf_path_states[pixel_index].accumulated_b = 0.0;
    wf_path_states[pixel_index].flags = 0u;

    // Initialize hit to invalid
    wf_hits[ray_index].distance = F32_INFINITY;
}

// Kernel 2: Trace rays (BVH traversal only)
@compute @workgroup_size(64, 1, 1)
fn wf_trace_rays(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let local_index = global_id.x;
    let active_count = atomicLoad(&wf_compact_state[WF_COMPACT_ACTIVE_COUNT]);

    if local_index >= active_count {
        return;
    }

    // Load ray from current source buffer
    let ray = wf_load_ray(local_index);
    let hit_record = hit(ray);

    // Store hit result (hits are indexed by local_index, single-buffered)
    wf_store_hit(local_index, hit_record);
}

// Kernel 3: Shade hits and extend paths (with integrated compaction)
// Uses WF_COMPACT_ACTIVE_COUNT as input count and WF_COMPACT_NEXT_COUNT for output count
@compute @workgroup_size(64, 1, 1)
fn wf_shade_and_extend(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let local_index = global_id.x;
    let active_count = atomicLoad(&wf_compact_state[WF_COMPACT_ACTIVE_COUNT]);
    ...
    // Load ray from source buffer
    let src_offset = wf_src_offset();
    let ray = wf_load_ray_at(src_offset + local_index);
    let pixel_id = wf_rays[src_offset + local_index].pixel_id;
    let bounce = wf_rays[src_offset + local_index].bounce;

    // Path state is indexed by pixel_id (not compacted)
    let pixel_index = wf_ray_index(wf_unpack_pixel_id(pixel_id));

    // Load hit from local_index (hits are indexed same as src rays)
    let hit_distance = wf_hits[local_index].distance;

    // Load path state from pixel index
    var throughput = vec3<f32>(
        wf_path_states[pixel_index].throughput_r,
        wf_path_states[pixel_index].throughput_g,
        wf_path_states[pixel_index].throughput_b,
    );
    var accumulated = vec3<f32>(
        wf_path_states[pixel_index].accumulated_r,
        wf_path_states[pixel_index].accumulated_g,
        wf_path_states[pixel_index].accumulated_b,
    );
    var seed = wf_path_states[pixel_index].rng_seed;

    // Handle miss: add environment radiance and terminate
    if hit_distance >= F32_INFINITY {
        let env_radiance = get_environment_radiance(ray);
        accumulated += throughput * env_radiance;
        wf_path_states[pixel_index].accumulated_r = accumulated.r;
        wf_path_states[pixel_index].accumulated_g = accumulated.g;
        wf_path_states[pixel_index].accumulated_b = accumulated.b;
        wf_path_states[pixel_index].flags |= POD_PATH_STATE_FLAG_TERMINATED;
        return;
    }

    // Reconstruct full hit record
    let hit_record = wf_load_hit(local_index, ray);

    // Compute hit details (material evaluation)
    let hit_details = compute_hit_details(hit_record);

    // Debug hooks for primary ray hit visualization (only on first bounce)
    if bounce == 0u {
        let pixel_xy = wf_unpack_pixel_id(pixel_id);
        debug_hook_post_primary_ray_hit(hit_record, vec2<i32>(pixel_xy));
        debug_hook_post_primary_ray_hit_details(hit_details, vec2<i32>(pixel_xy));
    }

    // Add emissive contribution
    accumulated += throughput * hit_details.surface_emissive;

    // Check max bounces
    if bounce >= frame_info.max_bounces {
        wf_path_states[pixel_index].accumulated_r = accumulated.r;
        wf_path_states[pixel_index].accumulated_g = accumulated.g;
        wf_path_states[pixel_index].accumulated_b = accumulated.b;
        wf_path_states[pixel_index].flags |= POD_PATH_STATE_FLAG_TERMINATED;
    ...
    let brdf_result = sample_brdf(hit_details, ray.direction, &seed);

    // Update throughput
    throughput *= brdf_result.weight;

    // Russian roulette
    if should_terminate_russian_roulette(throughput, &seed) {
        wf_path_states[pixel_index].accumulated_r = accumulated.r;
        wf_path_states[pixel_index].accumulated_g = accumulated.g;
        wf_path_states[pixel_index].accumulated_b = accumulated.b;
        wf_path_states[pixel_index].flags |= POD_PATH_STATE_FLAG_TERMINATED;
    ...
    // Compensate throughput for Russian roulette
    throughput = compensate_russian_roulette(throughput);

    // Generate new ray origin (offset to avoid self-intersection)
    let new_origin = offset_ray_origin(hit_details, brdf_result.direction);
    let new_ray = Ray(new_origin, brdf_result.direction);

    // COMPACTION: Atomically get new index in destination buffer
    let new_index = atomicAdd(&wf_compact_state[WF_COMPACT_NEXT_COUNT], 1u);

    // Store ray to destination buffer at compacted index
    let dst_offset = wf_dst_offset();
    wf_store_ray_at(dst_offset + new_index, new_ray, pixel_id, bounce + 1u);

    // Update path state (indexed by pixel)
    wf_path_states[pixel_index].throughput_r = throughput.r;
    wf_path_states[pixel_index].throughput_g = throughput.g;
    wf_path_states[pixel_index].throughput_b = throughput.b;
    wf_path_states[pixel_index].rng_seed = seed;
    wf_path_states[pixel_index].accumulated_r = accumulated.r;
    wf_path_states[pixel_index].accumulated_g = accumulated.g;
    wf_path_states[pixel_index].accumulated_b = accumulated.b;
...
// Kernel 4: Finalize and write to output
@compute @workgroup_size(8, 8, 1)
fn wf_finalize(@builtin(global_invocation_id) global_id: vec3<u32>) {
    if global_id.x >= frame_info.target_size_w_px || global_id.y >= frame_info.target_size_h_px {
        return;
    }
    ...
    // Load accumulated radiance
    let radiance = vec3<f32>(
        wf_path_states[ray_index].accumulated_r,
        wf_path_states[ray_index].accumulated_g,
        wf_path_states[ray_index].accumulated_b,
    );

    // For now, single sample per pixel in wavefront mode
    // Multi-sample support would require running the full pipeline multiple times
    // or restructuring the sample loop
    let pixel_color = vec4<f32>(radiance, 1.0);

    finish_frame(pixel_xy, pixel_color);
}

// Kernel 5: Swap buffers after shade_and_extend
// This prepares the compaction state for the next bounce iteration
@compute @workgroup_size(1, 1, 1)
fn wf_swap_buffers(@builtin(global_invocation_id) global_id: vec3<u32>) {
    // Only one thread should execute this
    if global_id.x != 0u {
        return;
    }

    // Move next_count to active_count for the next bounce
    let next_count = atomicLoad(&wf_compact_state[WF_COMPACT_NEXT_COUNT]);
    atomicStore(&wf_compact_state[WF_COMPACT_ACTIVE_COUNT], next_count);

    // Reset next_count for the next shade_and_extend pass
    atomicStore(&wf_compact_state[WF_COMPACT_NEXT_COUNT], 0u);

    // Toggle current buffer (0 -> 1, 1 -> 0)
    let current_buf = atomicLoad(&wf_compact_state[WF_COMPACT_CURRENT_BUF]);
    atomicStore(&wf_compact_state[WF_COMPACT_CURRENT_BUF], 1u - current_buf);
}

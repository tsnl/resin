...
type ElementKind = Literal["float", "uint"]
...
def etype(kind: ElementKind, nbytes: int) -> ElementType:
    match (kind, nbytes):
        case ("float", 4):
            return "f4"
        case ("float", 2):
...
def etype_kind(etype: ElementType) -> ElementKind:
    match etype:
        case "f4" | "f2":
            return "float"
        case "u4":
...
def etype_join_kind(etype1: ElementKind, etype2: ElementKind) -> ElementKind:
    if etype1 != etype2:

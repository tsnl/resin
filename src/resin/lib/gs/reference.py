...
def _cov3d_to_conic(
    ...
    *,
    ...
    # cov2d = J @ cov3d @ J^T (2x2 upper triangle)
    c00 = cov3d[0][0]
    c01 = cov3d[0][1]
    c02 = cov3d[0][2]
    c11 = cov3d[1][1]
...
def preprocess_gaussians_cpu(
    ...
    *,
    ...
    for i, mean in enumerate(cloud.means):
        ...
        conic = _cov3d_to_conic(cov3d, (cam[0], cam[1], cam[2]), focal_x=focal_x, focal_y=focal_y)
        if conic is None:
            continue
        c0, c1, c2 = conic
        det = c0 * c2 - c1 * c1
...
def blend_gaussians_cpu(
    *,
    ...
    for idx in order:
        ...
        x0 = max(0, int(math.floor(mx - 3.0 * math.sqrt(max(c0, c2)))))
        x1 = min(width, int(math.ceil(mx + 3.0 * math.sqrt(max(c0, c2)))))
        y0 = max(0, int(math.floor(my - 3.0 * math.sqrt(max(c0, c2)))))
        y1 = min(height, int(math.ceil(my + 3.0 * math.sqrt(max(c0, c2)))))
        ...
        for py in range(y0, y1):
            for px in range(x0, x1):
                ...
                image[offset] += r * alpha
                image[offset + 1] += g * alpha
                image[offset + 2] += b * alpha

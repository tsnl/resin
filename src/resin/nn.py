...
class Linear:
    ...
    def new(in_features: int, out_features: int, bias: bool = True) -> "Linear":
        weight = dsl.param(shape=(out_features, in_features), stype="f4")
        bias_node = dsl.param(shape=(out_features,), stype="f4") if bias else None
...
def relu(x: dsl.View) -> dsl.View:
    return x.max(dsl.const(0, stype=x.stype))

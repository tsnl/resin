...
from .gpu_util import Rgba8UnormTexture, StorageBuffer, StagingBuffer, TextureWrapper
...
class Draw2dQuad:
    ...
    dst_wh_px: Optional[Tuple[int, int]] = None
    src_xy_px: Optional[Tuple[int, int]] = None
    src_wh_px: Optional[Tuple[int, int]] = None
    fill_texture: Optional[wgpu.GPUTexture] = None
...
class QuadGroup:
    def __init__(
        ...
        self.device_buffer = StorageBuffer(
            device, self.capacity, "Draw2dFrame.QuadBatch.DeviceBuffer", PodQuad
        )
        self.staging_buffer = StagingBuffer(
            device, self.capacity, "Draw2dFrame.QuadBatch.StagingBuffer", PodQuad
        )
    ...
    def update(
        ...
        self.realloc_if_needed(
            device, len(data), quad_batch_bind_group_layout, quad_batch_sampler
        )

        self.staging_buffer.write(data)
        self.staging_buffer.copy_to_buffer(self.device_buffer, command_encoder)
...
class QuadBatchList:
    def __init__(self, quads: List[Draw2dQuad], framebuffer_size_wh: Tuple[int, int]):
        self.draw_ranges: List[Tuple[Optional[wgpu.GPUTexture], range]] = []
        self.bind_groups: Dict[Optional[wgpu.GPUTexture], List[PodQuad]] = {}
...
class Draw2dFrame:
    def __init__(self, device: wgpu.GPUDevice, target_size_wh: Tuple[int, int]):
        self.output_image = Rgba8UnormTexture(
            device, target_size_wh, "Draw2dFrame.OutputImage"
        )
    ...
    def acquire_quad_group(
        ...
        return self.quad_group_cache[key].update(
            device, command_encoder, bind_group_layout, sampler, quads
        )
...
class Draw2dRenderer:
    def __init__(
        ...
        self.default_white_texture = Rgba8UnormTexture(
            device, (32, 32), "Draw2dRenderer.DefaultWhiteTexture"
        )
        queue.write_texture(

...
See [Zed](zed.md), [Helix](helix.md), and
the [service guide](compiler-service.md) for complete setup.
...
to pick up formatting support; see the [Zed setup](zed.md#formatting)
or [Helix setup](helix.md#library-imports-and-formatting).
...
[CLI formatting guide](tools.md#formatting) for exit codes and file selection.
...
```sh
cargo test -p resin-client -p resin-protocol -p resin-server
cargo test -p resin --test lsp --test formatting --test format_cli
```

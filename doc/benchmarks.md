...
see the [compiler service benchmarks](service-benchmarks.md).
...
PowerShell described in the [guide](development.md#development).

## Workloads

Each file in [workloads/](../benchmarks/workloads) defines one kernel shared by the host and
...
```sh
cargo bench --bench cpu -- --list
cargo bench --bench cpu -- --workload branch_heavy --workload arithmetic --samples 40 --warmup 10 --json cpu.json --label baseline
cargo bench --bench gpu -- --list-devices
cargo bench --bench gpu -- --device 0 --workload branch_heavy --samples 40 --warmup 10 --json gpu.json --label baseline
```
...
```sh
cargo bench --bench cpu -- --size 65 --samples 2 --warmup 1
cargo bench --bench gpu -- --size 65 --samples 2 --warmup 1
```

...
| --- | --- | --- |
| [`$/math.resin`](../resin/math.resin) | `Complex<T>`, `complex`, `add`, `mul`, `squared`, `magnitude_squared`, `sqrt`, `sin`, `cos` | Arithmetic helpers borrow inputs. Shader transcendental operations currently use `float32`. |
...
| [`$/string.resin`](../resin/string.resin) | `String`, `string_from_str`, `string_from_bytes`, `fmt`, `print` | Host allocation and formatting; `str` is a distinct primitive literal view. |
| [`$/io.resin`](../resin/io.resin) | `io_stdin`, `io_stdout`, `io_stderr`, reads and writes | Host I/O with ordinary error unions. |
| [`$/console.resin`](../resin/console.resin) | `console_read_byte`, `console_read_line`, `console_print` | Host input with explicit EOF and error variants. |

...
`resin_frontend::Frontend::build_hir(source, loader)` resolves imports and returns an
`Arc<FrontendOutput>` containing completed HIR and editor facts. The concrete
...
`Frontend` owns its cache fields directly; `FrontendOutput` owns its retained products.

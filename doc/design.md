...
local per function and a tree of blocks with explicit `If`, `Loop`, `Yield`, and
`Return` terminators. Block IDs locate storage in an arena; child references express
unique ownership, never arbitrary jumps. Verification checks branch yields and loop

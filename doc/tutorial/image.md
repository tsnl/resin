...
`view:at_mut(index)` returns `RefMut<u8>` and assignment writes that element.
The span descriptor is borrowed read-only; its stored pointer supplies writable
access to the pixels. Indexing keeps ownership of the buffer with its owner. Host indexing checks bounds before

...
fn dependent_static_calls_and_references_apply_owner_and_method_arguments() {
    ...
    "#;
    let module = resin_lir::build_lir_all(&hir(source)).unwrap();
    assert_eq!(
        module
            .functions
            .iter()
...
fn dependent_failures_report_the_demanded_application() {
    for (declaration, body, needle) in [
        ...
        let error = resin_lir::build_lir_all(&hir(&source)).unwrap_err();
...
fn growing_dependent_method_applications_obey_the_function_limit() {
    ...
    let errors = resin_lir::build_lir_all_with_options(&tree, &options).unwrap_err();
    assert!(
        errors
            .iter()

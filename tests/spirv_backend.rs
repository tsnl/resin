...
fn compute_workgroup_size_specializes_execution_mode_and_wide_index_arithmetic() {
    let m = module(
        "export { kernel }; @compute_shader fn kernel(index: u64, output: PtrMut<u64>)  { output.* = index; }",
    );
...
fn shader_indexing_emits_no_bounds_checks() {
    for indexing in [
        ...
        let m = module(&format!(
            "export {{ kernel }}; import {{ \"$/span.resin\" }}; @compute_shader fn kernel(i: u64, output: PtrMut<u32>)  {{ let mut values = [u32(1), u32(2)]; let mut view = SpanMut<u32> {{ data = output, length = u64(2) }}; output.* = {indexing}; }}"
        ));
        let project = support::project::Project::new(&m, None).unwrap();
        let source = std::fs::read(project.generated.shaders()[0].unoptimized_spirv()).unwrap();
...
fn shader_calls_guard_only_helpers_with_emitted_failure_exits() {
    for (helpers, expression, checks, loads) in [
        ...
        let source = format!(
            "export {{ kernel }}; {helpers} @compute_shader fn kernel(i: u64, output: PtrMut<u64>)  {{ output.* = {expression}; }}"
        );
...
fn shader_helpers_can_propagate_and_handle_results() {
    let m = module(
        "export { kernel }; struct Bad { index: u32, } fn checked(i: u32) -> (u32 | Err<Bad>)  { if (i == u32(0)) { Err(Bad { index = i }) } else { (i) } } fn helper(i: u32) -> (u32 | Err<_>)  { let mut value = checked(i)?; (value + u32(1)) } @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { match (helper(i)) { u32(value) => { value }, Err(error) => { error.index } } }; }",
    );
    let project = support::project::Project::new(&m, None).unwrap();
    if let Some(frontend) = shaders::optimizer() {
        project
...
fn inferred_shader_results_lower_without_backend_inference() {
    let m = module(
        "export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>) -> _  { let mut i = u32(invocation); output.* = { let mut value: _; value = i + 1; value }; }",
    );
...
fn examples_helpers_and_control_flow_compile_to_spirv() {
    ...
    let modules = [
        ...
        (
            module(
                "export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { let mut x = i; x = x + u32(2); if (x < u32(4)) { x } else { x * u32(2) } }; }",
            ),
            Stage::Compute,
        ),
        (
            module(
                "export { kernel }; type Pixel = u64; @compute_shader fn kernel(i: Pixel, output: PtrMut<u32>)  { output.* = { u32(i + Pixel(u64(1))) }; }",
            ),
            Stage::Compute,
        ),
        (
            module(
                "export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { twice(i) }; } fn twice (i: u32) -> u32  { add(i, i) } fn add (a: u32, b: u32) -> u32  { a + b }",
            ),
            Stage::Compute,
        ),
        (
            module(
                "export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { let mut x = i; let mut n: u32 = 0; while (n < u32(3)) { let mut j: u32 = 0; while (j < n) { x = x + j; j = j + u32(1); }; n = n + u32(1); }; x }; }",
            ),
            Stage::Compute,
        ),
    ];
...
fn device_pointers_and_shared_roots_compile() {
    ...
    for (source, stage) in [
        (
            "export { kernel }; struct Node { value: u32, next: PtrMut<Node>, } fn select (a: PtrMut<Node>, b: PtrMut<Node>, i: u32) -> PtrMut<Node>  { if (i == u32(0)) { a } else { b } } @compute_shader fn kernel (invocation: u64, root: PtrMut<Node>) -> ()  { let mut i = u32(invocation); let mut p = select(root, root.next, i); p.value = u32(7); }",
            Stage::Compute,
        ),
        (
            "export { kernel }; import { \"$/span.resin\" }; struct Data { wide: u64, values: PtrMut<u32>, } @compute_shader fn kernel (invocation: u64, root: PtrMut<Data>) -> ()  { let mut i = u32(invocation); let mut p = root.values; let mut q: RefMut<u32> = device_index(p, u64(64), u64(i)).*; q = u32(3); root.wide = u64(4294967297); }",
            Stage::Compute,
        ),
        (
            "export { fragment }; struct Color { r: f32, g: f32, b: f32, a: f32, } struct Params { scale: f32, } @fragment_shader fn fragment (color: Color, root: PtrMut<Params>) -> Color  { Color { r = color.r * root.scale, g = color.g, b = color.b, a = color.a } }",
            Stage::Fragment,
        ),
    ] {
...
fn shader_addresses_cannot_hide_unsupported_layouts_or_escape_locals() {
    for (source, expected) in [
        (
            "export { kernel }; fn read(p: PtrMut<u32>) -> u32  { p.* } @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); let mut local = i; output.* = read(&local); }",
            "cannot take the address of a local value",
        ),
        (
            "export { kernel }; struct Data { flag: bool, } @compute_shader fn kernel (invocation: u64, root: PtrMut<Data>) -> ()  { let mut i = u32(invocation); () }",
            "no shared host/device layout",
        ),
        (
            "export { kernel }; @compute_shader fn kernel (invocation: u64, root: PtrMut<()>) -> ()  { let mut i = u32(invocation); () }",
            "no shared host/device layout",
        ),
        (
            "export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { let mut x = i; let mut p = &x; p.* }; }",
            "cannot take the address of a local value",
        ),
        (
            "export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { let mut x = i; u64(&x); i }; }",
            "cannot take the address of a local value",
        ),
        (
            "export { kernel }; fn helper (i: u32) -> Ref<u32>  { let mut x = i; x } @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { helper(i) }; }",
...
fn unsupported_shader_features_are_diagnosed() {
    for (source, expected) in [
        (
            "export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { helper(i, output) }; } fn helper (i: u32, output: PtrMut<u32>) -> u32  { kernel(u64(i), output); output.* }",
            "recursive shader call graph",
        ),
        (
            "export { kernel }; extern { \"stdlib.h\": { fn abs (i: i32) -> i32; } }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { abs(1); i }; }",
            "foreign",
        ),
        (
            "export { kernel }; intrinsic \"format_bytes\" fn render<A>(data: Ptr<u8>, length: u64, args: A) -> StrongOwner; struct Root { data: PtrMut<u8>, length: u64, } @compute_shader fn kernel(invocation: u64, root: PtrMut<Root>)  { let mut text = render(root.data, root.length, ()); }",
            "shader cannot consume managed values",
        ),
        (
            "export { kernel }; fn helper (i: u32) -> u32  { i } @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = { let mut f = helper; f(i) }; }",
...
fn imported_backend_errors_retain_expression_origins() {
    ...
    let entry = Source::new(
        "main.resin",
        "export { kernel }; import { \"helper.resin\" }; @compute_shader fn kernel(invocation: u64, p: PtrMut<u32>)  { p.* = helper(u32(invocation)); }",
    );
...
fn managed_fields_are_opaque_until_consumed_by_a_shader() {
    ...
    let m = module(&format!(
        "{prefix} @compute_shader fn kernel(invocation: u64, root: PtrMut<Root>)  {{ let mut i = u32(invocation); root.result = i; let mut address = &root.owner; }}"
    ));
    let project = support::project::Project::new(&m, None).unwrap();
    if let Some(frontend) = shaders::optimizer() {
        project.build(&toolchain::spirv(&frontend)).unwrap();
    ...
    for body in [
        ...
        let error = pipeline::shader_error(&format!(
            "{prefix} @compute_shader fn kernel(invocation: u64, root: PtrMut<Root>)  {{ let mut i = u32(invocation); {body} }}"
        ));
        assert!(
...
fn options_of_plain_values_work_in_shaders() {
    let m = module(
        "export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); let mut value: u32 | None; value = if (i == u32(0)) { u32(42) } else { None }; output.* = match (value) { u32(n) => { n }, None => { u32(0) } }; }",
    );
    let project = support::project::Project::new(&m, None).unwrap();
    if let Some(frontend) = shaders::optimizer() {
        project.build(&toolchain::spirv(&frontend)).unwrap();
...
fn literal_strings_report_the_missing_shader_storage_support() {
    let error = pipeline::shader_error(
        r#"export { kernel }; @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); let mut text = "abc"; output.* = u32(text.length); }"#,
    );
    assert!(
        error
            .to_string()
...
fn compute_index_uses_wide_arithmetic_and_indexes_spans_directly() {
    let m = module(
        "export { kernel }; import { \"$/span.resin\" }; @compute_shader fn kernel(index: u64, output: PtrMut<SpanMut<u64>>)  { if (index < output.length) { output.*:at_mut(index) = index; }; }",
    );
    let project = support::project::Project::new(&m, None).unwrap();
    let source = std::fs::read(project.generated.shaders()[0].unoptimized_spirv()).unwrap();
...
fn sequential_conditionals_and_error_propagation_preserve_structured_control() {
    ...
    source.push_str("(result) } @compute_shader fn kernel(i: u64, output: PtrMut<u32>)  { output.* = match (helper(u32(i))) { u32(value) => { value }, Err(error) => { u32(99) } }; }");
    let project = support::project::Project::new(&module(&source), None).unwrap();
    let source = std::fs::read(project.generated.shaders()[0].unoptimized_spirv()).unwrap();
    assert_eq!(
        instructions(&source, 246).count(),
...
fn local_reference_specialization_bounds_nested_emission() {
    ...
    source.push_str("@compute_shader fn kernel(index: u64, output: PtrMut<u32>) { let mut value: u32 = 0; helper_0(value); output.* = value; }");
    let error = pipeline::shader_error(&source);
    assert!(
        error
            .to_string()

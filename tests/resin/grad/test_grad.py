...
class TestAccessorAdjoint:
    def test_broadcast_reduces(self) -> None:
        x = param(shape=(3,), dtype="f4", label="x")
        y = x.broadcast((4,))
        g = param(shape=y.shape, dtype="f4", label="g")
    ...
    def test_scalar_broadcast(self) -> None:
        x = param(shape=(), dtype="f4", label="x")
        y = x.broadcast((2, 3))
        g = param(shape=y.shape, dtype="f4", label="g")
        grad_x = grad.accessor_adjoint(y, g)
...
class TestReductionDfDo:
    def _make(self, axes, operator):
        x = param(shape=(2, 3), dtype="f4", label="x")
        n = x.reduce(axes=axes, operator=operator)
        df_dn = param(shape=n.shape, dtype="f4", label="df_dn")
        return x, n, df_dn
...
class TestGrad:
    ...
    def test_matmul_grad_shapes(self) -> None:
        x = param(shape=(2, 3), dtype="f4", label="x")
        w = param(shape=(3, 4), dtype="f4", label="w")
    ...
    def test_broadcast_and_reduce_grad(self) -> None:
        p = param(shape=(3,), dtype="f4", label="p")
        b = param(shape=(2, 3), dtype="f4", label="b")
    ...
    def test_copy_then_sum_grad(self) -> None:
        x = param(shape=(4,), dtype="f4", label="x")

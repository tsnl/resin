...
class TestCompact:
    def test_compact_after_slice(self) -> None:
        ...
        expected = textwrap.dedent(
            """
            remap(direction='gather', keys='accessor', operator=None, woffset=0, wpitch=(), source_shape=None) :: f4(1, 3)
    ...
    def test_compact_noop_on_contiguous(self) -> None:
        ...
        expected = textwrap.dedent(
            """
            remap(direction='gather', keys='accessor', operator=None, woffset=0, wpitch=(), source_shape=None) :: f4(3,)
            └ const(value=[1, 2, 3]) :: f4(3,)
            """
        )

...
struct First { value: i32, }
        ...
        struct Node { next: PtrMut<Node>, value: Optional, }
    ...
    let (second_index, second_definition) = module
        .types
        .iter()
        .enumerate()
        .find(|(_, definition)| {
            definition
                .name()
                .is_some_and(|name| name.as_ref() == "Second")
        })
        .unwrap();
    let second = second_definition.ty(TypeId::from_index(second_index));
    let first_index = module.types.id(&first).unwrap().index();
...
fn both_emitters_use_payload_table_indices_as_union_tags() {
    let mut module = support::module(
        ...
        @compute_shader fn kernel(invocation: u64, output: PtrMut<u32>)  { let mut i = u32(invocation); output.* = choose(i)!; }
        "#,
    );

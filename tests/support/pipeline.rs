...
pub fn generate(file: &ast::SourceFile) -> Result<lir::Module, GenerateError> {
    let tree = resin_hir::generate(file)?;
    let module = lir::generate(&tree).map_err(|error| error.error)?;
...
pub fn generate_program(program: &ast::Program) -> Result<lir::Module, ast::SourceError> {
    let tree = resin_hir::generate_program(program)?;
    let module = lir::generate(&tree).map_err(|error| {
    ...
    resin_lir_verifier::VerifiedModule::new(module)
        .map(|v| v.into_module())
        .map_err(|error| ast::SourceError::new(Default::default(), None, error.to_string()))
}
pub fn load(path: &Path) -> Result<ast::Program, ast::SourceError> {
...
) -> Result<ast::Program, ast::SourceError> {

use resin::types::{RecordField, Ty, TypeDef, TypeErrorKind, TypeId, TyperContext};
fn record(ty: Ty) -> Ty {
    Ty::Record {
...
fn nominal_nonrecords_are_rejected_at_creation_and_verification() {
    for body in [
        ...
        let module = resin::lir::Module {
            types: vec![TypeDef::new("Invalid", body)].into(),
            ..Default::default()
        };
        assert_eq!(
            resin::lir_verifier::verify(&module).unwrap_err().kind,
            resin::lir_verifier::VerifyErrorKind::NominalTypeMustBeRecord { definition }
        );
    }
}
...
fn inline_dependencies_require_completion_but_indirect_fields_can_be_pending() {
    ...
    resin::lir_verifier::verify(&resin::lir::Module {

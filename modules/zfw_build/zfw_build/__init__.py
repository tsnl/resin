...
def emit_generated_init_py(bundled_data_path: Path):
    with open(bundled_data_path / "__init__.py", "w") as f:
        print("# Auto-generated by zfw_build. Do not edit.", file=f)
        print(file=f)
        print('__all__ = ["BUNDLED_DATA_PATH"]', file=f)
        print(file=f)
        print("from pathlib import Path", file=f)
        print(file=f)
        print("BUNDLED_DATA_PATH = Path(__file__).absolute().parent", file=f)
...
def main():
    ...
    # __init__.py
    emit_generated_init_py(args.package / package_name / "bundled_data")

...
fn unsaved_unicode_buffers_support_features_edits_and_clean_shutdown() {
    ...
    let source = "main (argument: int) -> int = {\r\n  // 😀\r\n  value = argument + 1;\r\n  /* 😀 */ value\r\n};\r\n";
    ...
    client.change(&uri, 3, "main (argument: int) -> int = { arg");
    client.diagnostics(&uri, Some(3), true);
    let completion = client.request("textDocument/completion", at(&uri, 0, 33));
    assert!(
        completion["items"]
            .as_array()
            .unwrap()
...
fn dependency_overlays_close_and_disk_changes_refresh_consumers() {
    ...
    std::fs::write(&library, "export { answer }; answer = 1; private = 2;").unwrap();
    let mut client = Client::start(root, Value::Null);
    let main_uri = uri(&root.join("main.resin"));
    let lib_uri = uri(&library);
    let source = "import { \"lib.resin\" }; main () -> int = { answer };";
    ...
    client.open(&lib_uri, "export { renamed }; renamed = 2;");
    ...
    std::fs::write(&library, "export { answer }; answer = ;").unwrap();
    client.notify(
        "workspace/didChangeWatchedFiles",
        json!({"changes": [{"uri": lib_uri, "type": 2}]}),
    );
    client.diagnostics(&lib_uri, None, true);
    std::fs::write(&library, "export { answer }; answer = 3;").unwrap();
    client.notify(
        "workspace/didChangeWatchedFiles",
        json!({"changes": [{"uri": lib_uri, "type": 2}]}),
    );
...
fn stdlib_override_and_rapid_versions_use_the_latest_snapshot() {
    ...
    std::fs::write(
        ...
        "export { standard }; standard = 1;",
    )
    .unwrap();
    let mut client = Client::start(temp.path(), json!({"stdlibPath": stdlib}));
    let uri = uri(&temp.path().join("main.resin"));
    client.open(&uri, "import { \"std/custom.resin\" }; standard;");
    client.diagnostics(&uri, Some(1), false);
    for version in 2..50 {
        client.change(&uri, version, &format!("value{version} = missing;"));
    }
    client.change(&uri, 50, "final = 1; final;");
    client.diagnostics(&uri, Some(50), false);
    let hover = client.request("textDocument/hover", at(&uri, 0, 12));
    assert!(
        hover["contents"]["value"]
            .as_str()
            .unwrap()

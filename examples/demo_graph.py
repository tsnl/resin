...
ConstNode.new(42, dtype="fp32").debug_print(out=sys.stderr)

section("1D constant")
ConstNode.new([1, 2, 3], dtype="fp32").debug_print(out=sys.stderr)

section("2D constant")
ConstNode.new([[1, 2], [3, 4]], dtype="fp32").debug_print(out=sys.stderr)

section("fp16 constant")
ConstNode.new([1, 2], dtype="fp16").debug_print(out=sys.stderr)
...
ConstNode.new([1, 2], dtype="fp32").exp().log().debug_print(out=sys.stderr)

section("Deep chain: exp.log.exp.log")
ConstNode.new([1, 2, 3, 4], dtype="fp32").exp().log().exp().log().debug_print(
    out=sys.stderr
)
...
ConstNode.new([[1, 2, 3], [4, 5, 6]], dtype="fp32").reduce(
    axis=1, operator="add"
).debug_print(out=sys.stderr)

# Indexing (must use tuple keys)
section("Integer index")
ConstNode.new([[1, 2, 3], [4, 5, 6]], dtype="fp32")[(0, slice(None))].debug_print(
    out=sys.stderr
)

section("Slice with step")
ConstNode.new([1, 2, 3, 4, 5, 6], dtype="fp32")[(slice(None, None, 2),)].debug_print(
    out=sys.stderr
)

section("Multi-dim index")
ConstNode.new([[1, 2, 3], [4, 5, 6]], dtype="fp32")[(1, slice(1, 3))].debug_print(
    out=sys.stderr
)

# Permute
section("Permute (transpose)")
ConstNode.new([[1, 2, 3], [4, 5, 6]], dtype="fp32").permute((1, 0)).debug_print(
    out=sys.stderr
)
...
ConstNode.new([1, 2, 3], dtype="fp32").max(0).debug_print(out=sys.stderr)
...
# Param node
section("Param node")
ParamNode.new(shape=(4,), dtype="fp32", label="weights").debug_print(out=sys.stderr)

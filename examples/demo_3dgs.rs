...
fn main() {
    ...
    let image = match backend.as_str() {
        #[cfg(feature = "cpu")]
        "cpu" => render_with(resin::jit::backends::cpu::CpuJit, &data, &camera),
        #[cfg(feature = "wgpu")]
        "wgpu" => render_with(resin::jit::backends::wgpu::WgpuJit, &data, &camera),
...
fn render_auto(data: &CloudData, camera: &Camera) -> Vec<f32> {
    ...
    if resin::jit::backends::wgpu::shared_context_available() {
        ...
        return render_with(resin::jit::backends::wgpu::WgpuJit, data, camera);
    }
    #[cfg(feature = "cpu")]
    {
        println!("backend: cpu");
        return render_with(resin::jit::backends::cpu::CpuJit, data, camera);
    }
    #[allow(unreachable_code)]
    {
        panic!("no backend available (build with `cpu` and/or `wgpu` features)")
    }
}
...
fn render_with<J: Jit>(jit: J, data: &CloudData, camera: &Camera) -> Vec<f32> {
    ...
    f.call(&cloud).expect("render").to_f32()
}

...
| `resin-dsl` | `View`, graph nodes, ops, prelude |
...
Public `View` holds a private `Arc<NodeInner>` plus an `Accessor`. Callers never see `Arc`.
...
- Nodes are immutable after creation; the graph is a **DAG**.
...
Host APIs that today take Python `PyTree[T]` use a **`ParamTree` trait** — not a live `enum` of Object/Array/Leaf. Module shape is the concrete type (`Linear`, `Mlp`, `Vec<…>`); collections are impls.
...
`IrProgramBuilder` memos `*const NodeInner → IrBuffer` and view identity → `IrBufferView`. Kernels: elementwise RPN, matmul, reduction, remap. Output: owned, index-based, serde-friendly `IrProgram`. Compile facade registers params/sinks through `ParamTree::flatten`.
...
| **P1** | `resin-core` (`ParamTree`, accessor, etype) + `resin-dsl` (`View` + ops) |
...
| **P5** | Compile/bind helpers, `sgd`, `resin-nn` |
